...
class Category extends CI_Controller 
{
	function __construct() 
	{
		...
		$this->load->model('backend/admin/Model_category');
	...
	public function index()
	{
		...
		$this->load->view('backend/admin/view_category',$data);
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
				...
				redirect(base_url().'backend/admin/category');
		    } 
		    else
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url().'backend/admin/category/add');
		    }
            
        } else {            
            $this->load->view('backend/backend/admin/view_header',$data);
			$this->load->view('backend/backend/admin/view_category_add',$data);
			$this->load->view('backend/backend/admin/view_footer');
        }
		...
		if(isset($_POST['form1'])) 
		{
		    ...
		    if($valid == 1) 
		    {
		    	...
		    	if($path == '') {
		        	...
		        	$form_data['category_banner'] = $final_name;
		    ...
		    {
		    	$this->session->set_flashdata('error',$error);
				redirect(base_url().'backend/admin/category/edit'.$id);
		    }
           
		} else {
			$data['category'] = $this->Model_category->get_category($id);
	       	$this->load->view('backend/admin/view_header',$data);
			$this->load->view('backend/admin/view_category_edit',$data);
			$this->load->view('backend/admin/view_footer');
		}
