...
class Model_common extends CI_Model 
{
    ...
    {
        $sql = "SELECT * from tbl_authorize WHERE controller = ?";
        $query = $this->db->query($sql,$controller);
	...
	public function get_setting_shop_data()
    {
    ...
    public function extension_check_photo($param) {
        ...
        $ext = array('svg','SVG','jpg','JPG','png','PNG','jpeg','JPEG','gif','GIF','ico','ICO');
    ...
    public function extension_check_file($param) {
        $ext = array('pdf','zip','PDF', 'ZIP', 'gif', 'GIF');
    	if( in_array($param,$ext)) {
    		return true;
    	} else {
    		return false;
