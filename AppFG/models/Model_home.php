...
class Model_home extends CI_Model 
{
    ...
    {
        $query = $this->db->query("SELECT * FROM tbl_service WHERE status=? ORDER BY id ASC",array('Show'));
        return $query->result_array();
    }
    ...
    public function all_how_we_works()
    {
        $query = $this->db->query("SELECT * FROM tbl_how_we_works ORDER BY id ASC");
        return $query->result_array();
    }
