<?php
defined('BASEPATH') or exit('No direct script access allowed');

class Model_covid_test extends CI_Model
{

    function get_auto_increment_id()
    {
        $sql = "SHOW TABLE STATUS LIKE 'tbl_schnell_test_form'";
        $query = $this->db->query($sql);
        return $query->result_array();
    }

    function show()
    {
        $sql = "SELECT * FROM tbl_schnell_test_form ORDER BY id ASC";
        $query = $this->db->query($sql);
        return $query->result_array();
    }

    function add($data)
    {
        $this->db->insert('tbl_schnell_test_form', $data);
        return $this->db->insert_id();
    }

    function update($id, $data)
    {
        $this->db->where('id', $id);
        $this->db->update('tbl_schnell_test_form', $data);
    }

    function delete($id)
    {
        $this->db->where('id', $id);
        $this->db->delete('tbl_schnell_test_form');
    }

    function get_covid_test($id)
    {
        $sql = 'SELECT * FROM tbl_schnell_test_form WHERE id=?';
        $query = $this->db->query($sql, array($id));
        return $query->first_row('array');
    }

    function covid_test_check($id)
    {
        $sql = 'SELECT * FROM tbl_schnell_test_form WHERE id=?';
        $query = $this->db->query($sql, array($id));
        return $query->first_row('array');
    }

    function action($id,$data)
    {
        $this->db->where('id', $id);
        $this->db->update('tbl_schnell_test_form', $data);
        return $this->db->affected_rows();
    }

    //datatable first step
    function ajax_covid_test_totalRecordwithFilter ($searchQuery)
    {
        ## Total number of record with filtering
        $this->db->select('count(*) as allcount');
        if ($searchQuery != '')
            $this->db->where($searchQuery);
        $records = $this->db->get('tbl_schnell_test_form')->result();
        return $records[0]->allcount;
    }

    //datatable second step
    function ajax_covid_test_records ($searchQuery,$columnName,$columnSortOrder,$rowperpage,$start)
    {
        ## Fetch records
        $this->db->select('*');

        if ($searchQuery != '')
            $this->db->where($searchQuery);

        $this->db->order_by($columnName, $columnSortOrder);
        $this->db->limit($rowperpage, $start);
        return $this->db->get('tbl_schnell_test_form')->result();
    }

    //datatable third step
    function ajax_covid_test_totalRecords()
    {
        ## Total number of records without filtering
        $this->db->select('count(*) as allcount');
        $records = $this->db->get('tbl_schnell_test_form')->result();
        return $records[0]->allcount;
    }

}
