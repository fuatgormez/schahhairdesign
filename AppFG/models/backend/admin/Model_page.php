...
class Model_page extends CI_Model 
{
    ...
    {
        $this->db->where('id',1);
        $this->db->update('tbl_page_job',$data);
    }
    ...
    public function update_impressum($data)
    {
        $this->db->where('id',1);
        $this->db->update('tbl_page_impressum',$data);
    }

    public function show_datenschutz()
    {
        $query = $this->db->query("SELECT * FROM tbl_page_datenschutz WHERE id=1");
        return $query->first_row('array');
    }

    public function update_datenschutz($data)
    {
        $this->db->where('id',1);
        $this->db->update('tbl_page_datenschutz',$data);
    }
    ...
    public function update_agb($data)
    {
        $this->db->where('id',1);
        $this->db->update('tbl_page_agb',$data);
    }
    
    public function show_wiederruf()
    {
        $query = $this->db->query("SELECT * FROM tbl_page_wiederruf WHERE id=1");
        return $query->first_row('array');
    }

    public function update_wiederruf($data)
    {
        $this->db->where('id',1);
        $this->db->update('tbl_page_wiederruf',$data);
    }
